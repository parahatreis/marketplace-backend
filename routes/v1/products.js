...
// @desc Create Product
// @access Private(admin)
router.post('/store', storeAdminAuth, async (req, res) => {
...
router.get('/subcategorie/:subcategorie_id', async (req, res) => {
  ...
  try {
    ...
    if (req.query.size) {
      ...
      const findSize = await SizeName.findOne({
        where: {
          size_name_id: size
        }
      });
    ...
    if (req.params.subcategorie_id) {
      const findSubcategorie = await SubCategorie.findOne({
        where: {
...
router.get('/brand/:brand_id', async (req, res) => {
  ...
  try {
    ...
    if (req.query.subcategorie) {
      const findSubcategorie = await SubCategorie.findOne({
        where: {
          subcategorie_id: req.query.subcategorie
        }
      });
      if (!findSubcategorie) return res.status(404).json({
        msg: 'Subcategorie not found!'
      })
    ...
    if (req.params.brand_id) {
      ...
      if (!findBrand) return res.status(404).json({
        msg: 'Brand not found!'
      })
      brand = findBrand.id
    }
